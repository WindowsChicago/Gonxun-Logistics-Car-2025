        ...
        while(True):
            ...
            if mode == 21:  # 收到信号0x11，识别红色物块
                ...
                for gobj in objects:
                    ...
                    img.draw_string(gpos[0], gpos[1], "%s : %.2f" % (labels[gobj.classid()], gobj.value()), scale=2,color=(0, 255, 0))  # 检测识别物体是否是所训练的模型
                ...
                for bobj in objects:
                    ...
                    img.draw_string(bpos[0], bpos[1], "%s : %.2f" % (labels[bobj.classid()], bobj.value()), scale=2,color=(0, 255, 0))  # 检测识别物体是否是所训练的模型
            ...
            if mode == 34:  # 收到信号0x22，识别色环
                ...
                if objects:
                    for obj in objects:
                        ...
                        print(pos)
                        img.draw_rectangle(pos)
                        img.draw_string(pos[0], pos[1], "%s : %.2f" % (labels[obj.classid()], obj.value()), scale=2,color=(0, 255, 0))
                        x, y, width, height = obj.rect()
                        cx = x + width // 2  # 用识别物体矩形框中心点代替物体中心点
                        cy = y + height // 2
                        x_offset = cx - 112 #随便给的
            ...
            if mode == 35:  # 收到信号0x23，识别蓝色色环
                ...
                if objects:
                    for obj in objects:
                        ...
                        #print(pos)
                        img.draw_rectangle(pos)
                        img.draw_string(pos[0], pos[1], "%s : %.2f" % (labels[obj.classid()], obj.value()), scale=2,color=(0, 0, 255))  # 检测识别物体是否是所训练的模型
                        x, y, width, height = obj.rect()
                        cx = x + width // 2  # 用识别物体矩形框中心点代替物体中心点
                        cy = y + height // 2
