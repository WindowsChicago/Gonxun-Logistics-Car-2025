    ...
    while True:
        ...
        if mode == 21:# 0x15，串口发送端示例：5A 15 15 15 15 15
            ...
            for r_blob in r_blobs:
                ...
                print("red block (", int(cx), ",", int(cy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）

                sending_data(1,int(cx),int(cy),int(0),0x6b);                   #通过串口发送数据给STM32
            ...
            for g_blob in g_blobs:
                ...
                print("green block (", int(gcx), ",", int(gcy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）

                sending_data(2,int(),int(ggx),int(gper),0x6b);                   #通过串口发送数据给STM32
            ...
            for b_blob in b_blobs:
                ...
                print("blue block (", int(bcx), ",", int(bcy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）
