...
# # 定义颜色的HSV阈值，适用于红、绿、蓝色环
# # 红色环的HSV范围
# RED_THRESH = (30, 100, 15, 127, 15, 127)
# # 绿色环的HSV范围
# GREEN_THRESH = (30, 100, -64, 64, -64, 64)
# # 蓝色环的HSV范围
# BLUE_THRESH = (30, 100, -128, 127, -128, 0)

    #圆环识别
    if data == 33:  # 0x21 自适应识别圆环

        #r_bin = img.to_grayscale()
        #g_bin = img.to_grayscale()
        #b_bin = img.to_grayscale()

        r_bin = img.binary([(0, 76, 7, 63, 42, -11)])           #将图像根据阈值进行二值化处理
        img.dilate(2)#将图像进行膨胀 使得色环糊成整体
        r_blobs = img.find_blobs([(0, 76, 7, 63, 42, -11)],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))  #寻找二值化后的色环
        # 标记红色环
        r_detected = False
        for blob in r_blobs:
            ...
            img.draw_cross(blob.cx(), blob.cy())  # 绘制交叉线
            r_detected = True

        g_bin = img1.binary([(0, 80,-70,-10, -0, 30)])
        img1.dilate(2)#将图像进行膨胀 使得色环糊成整体
        g_blobs = img1.find_blobs([(0, 80,-70,-10, -0, 30)],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))
                # 标记绿色环
        g_detected = False
        for blob in g_blobs:
            #blob = max(r_blobs, key=lambda b: b.pixels())
            ...
            cy = blob.cy() - img1.height() // 2
            sy = -cy

            img1.draw_rectangle(blob.rect())  # 绘制矩形框
            #img.draw_rectangle(r_blobs.rect())                     #在图像上框住色块
            ...
            img1.draw_cross(blob.cx(), blob.cy())  # 绘制交叉线
            g_detected = True

        b_bin = img2.binary([(0, 76,-2, 30, -6, -73)])
        img2.dilate(2)#将图像进行膨胀 使得色环糊成整体
        b_blobs = img2.find_blobs([(0, 76,-2, 30, -6, -73)],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))
        ...
        for blob in b_blobs:
            ...
            cy = blob.cy() - img2.height() // 2
            sy = -cy

            img2.draw_rectangle(blob.rect())  # 绘制矩形框
            #img.draw_rectangle(r_blobs.rect())                     #在图像上框住色块
            ...
            img2.draw_cross(blob.cx(), blob.cy())  # 绘制交叉线
            b_detected = True
    ...
    # # 查找红色环
    # red_blobs = img.find_blobs([RED_THRESH], area_threshold=100, merge=True)
    # # 查找绿色环
    # green_blobs = img.find_blobs([GREEN_THRESH], area_threshold=100, merge=True)
    # # 查找蓝色环
    # blue_blobs = img.find_blobs([BLUE_THRESH], area_threshold=100, merge=True)
