...
#色块颜色阈值范围定义（红，绿，蓝，灰）
color_thr_legacy = [(0, 60, 41, 5, 0, 65),(20,60,-70,-10,-0,30),(0, 61, 0, 52, 0, -44),(100,255)]
color_thr_old = [(0, 60, 41, 5, 0, 65),(20,60,-70,-10,-0,30),(8, 45, -13, 28, -56, -23),(100,255)]
color_thr = [(0, 60, 41, 5, 0, 65),(20,60,-70,-10,-0,30),(15, 43, -2, 11, -4, -27),(100,255)]
#色环颜色阈值范围定义（红，绿，蓝）
...
#从串口接收数据
    ...
    if data == 21:
        ...
        for g_blob in g_blobs:
            ...
            sending_signal(1,int(gsy),int(ggx),int(gper),0x6b);                   #通过串口发送数据给STM32

            img.draw_string(100,2, ("green"), color=(0,128,0), scale=3)     #在屏幕上展示要识别色块序号
            g_detected = True

        if not r_detected and not g_detected:
            print("No Ring detected")
        # 延时，避免程序过快执行
        time.sleep(0.1)
