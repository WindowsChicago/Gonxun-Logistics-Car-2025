    ...
    while True:
        ...
        if mode == 33:  # 0x21 识别红色圆环，6字节，每个字节为16进制数据，串口发送端示例：5A 21 21 21 21 21
            ...
            if r_blobs:
                ...
                print("red ring (", int(rsx), ",", int(rsy), ",",4, ")")      #在缓存区展示偏差值（用于调试）
        ...
        if mode == 34:  # 0x22 识别绿色圆环，串口发送端示例：5A 22 22 22 22 22
            ...
            if g_blobs:
                ...
                #img.draw_rectangle(r_blobs.rect())
                print("green ring (", int(gsx), ",", int(gsy), ",",4, ")")      #在缓存区展示偏差值（用于调试）

                sending_data(2,int(gsx),int(gsy),int(0),0x5b);                  #通过串口发送数据给STM32
        ...
        if mode == 35:  # 0x23 识别蓝色圆环，串口发送端示例：5A 22 22 22 22 22
            ...
            if b_blobs:
                ...
                #img.draw_rectangle(r_blobs.rect())
                print("blue ring (", int(bsx), ",", int(bsy), ",",4, ")")      #在缓存区展示偏差值（用于调试）

                sending_data(3,int(bsx),int(bsy),0,0x5c);              #通过串口发送数据给STM32
        ...
        if mode == 21:# 0x15，串口发送端示例：5A 15 15 15 15 15
            ...
            for r_blob in r_blobs:
                ...
                print("red block (", int(rsx), ",", int(rsy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）
            ...
            for g_blob in g_blobs:
                ...
                print("green block (", int(gsx), ",", int(gsy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）

                sending_data(2,int(gsx),int(gsy),int(0),0x6b);                   #通过串口发送数据给STM32
            ...
            for b_blob in b_blobs:
                ...
                print("blue block (", int(bsx), ",", int(bsy), ",",int(0), ")")      #在缓存区展示偏差值（用于调试）

                sending_data(3,int(bsx),int(bsy),int(0),0x6b);                   #通过串口发送数据给STM32
