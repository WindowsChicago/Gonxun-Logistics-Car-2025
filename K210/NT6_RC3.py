...
# BLUE_THRESH = (30, 100, -128, 127, -128, 0)
    ...
    if data == 33:  # 0x21 自适应识别圆环
        ...
        #g_bin = img.binary([(0, 80,-70,-10, -0, 30)])
        #b_bin = img.binary([(0, 76,-2, 30, -6, -73)])
    ...
    # blue_blobs = img.find_blobs([BLUE_THRESH], area_threshold=100, merge=True)
        ...
        r_blobs = img.find_blobs([color_thr[3]],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))  #寻找二值化后的色环
        g_blobs = img.find_blobs([color_thr[3]],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))
        b_blobs = img.find_blobs([color_thr[3]],area_threshold=150,pixels_threshold=3000,merge=True,roi=(0,0,320,240))
